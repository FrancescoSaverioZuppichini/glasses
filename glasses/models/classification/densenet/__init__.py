...
from ..resnet import ResNetEncoder, ResNetHead
            ...
            OrderedDict(
                {
                    ...
                    "act": activation(),
                    "conv": Conv2dPad(
                        in_features, out_features, kernel_size=3, **kwargs
                    ),
                }
            )
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.block = nn.Sequential(
            ...
            *[
                block(grow_rate * i + in_features, grow_rate, **kwargs)
                for i in range(n)
            ],
            # reduce the output features by a factor of 2
            transition_block(self.out_features, **kwargs)
            if transition_block
            else nn.Identity()
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        for deepth in depths[:-1]:
            self.layers.append(
                DenseNetLayer(start_features, grow_rate, deepth, block=block, **kwargs)
            )
            # in each layer the start_features are equal the features we have so far + the number of layer multiplied by the grow rate
            start_features += deepth * grow_rate
            start_features //= 2
            self.widths.append(start_features)

        self.widths.append(start_features + depths[-1] * grow_rate)
