...
class RegNetScaler:
    ...
    def __call__(
        self, w_0: float, w_a: float, w_m: float, depth: int, group_w: int, q: int = 8
    ):
...
class RegNetXBotteneckBlock(ResNetBottleneckBlock):
    ...
    def __init__(
        self, in_features: int, out_features: int, groups_width: int = 1, **kwargs
    ):
        super().__init__(
            in_features,
            out_features,
...
class RegNetYBotteneckBlock(RegNetXBotteneckBlock):
    ...
    def __init__(
        self, in_features: int, out_features: int, reduction: int = 4, **kwargs
    ):
...
class RegNet(ResNet):
    ...
    def regnetx_002(cls, *args, **kwargs):
        ...
        return cls(
            *args, depths=depths, widths=widths, groups_width=groups_width, **kwargs
        )
